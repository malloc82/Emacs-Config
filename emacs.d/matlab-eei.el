...
(defgroup matlab-eei nil
  "Debugger interface customization group."
  :group 'extensions)


(defcustom matlab-eei-display-variable-tips-p t
  "If this variable is on (nonnil) and the debugger is active, Emacs
displays the current value of the variable at point as a tooltip when
you move the mouse.

Note: use Emacs's standard tooltip customization variables (e.g.,
`tooltip-delay', to define the appearance, location, and timing of the
tooltip displays."
  :type 'boolean
  :group 'matlab-eei)
...
(defun matlab-eei-setup-matlab-mode ()
  ...
  ;; Set up variable tooltips.
  (unless (or
           (featurep 'xemacs)
           (not (fboundp 'x-show-tip)))    
    (require 'tooltip)
    (tooltip-mode 1)
    (add-hook 'tooltip-hook 'matlab-eei-display-variable-tooltip nil t)
    (set (make-local-variable 'tooltip-gud-modes) '(matlab-mode))
    (set (make-local-variable 'tooltip-gud-tips-p) t)))
...
(defun matlab-eei-debug-mode-status (on)
  "Sets debug mode status flag to ON."
  ;; (message "debug mode status %s" on)
  (setq matlab-eei-matlab-in-debug-mode-p on))
...
until the user saves the buffer."
  ...
  (if (not (buffer-modified-p))
      (matlab-eei-send-mcmd
       (format "request_set_breakpoint %s %d" 
               ;; Replace spaces in file names with tabs.
               ;; The Java side must restore the spaces.
...
at LINE in FILE be cleared."
  (if (buffer-modified-p)
    ...
    (matlab-eei-send-mcmd
     (format "request_clear_breakpoint %s %d" 
             ;; Replace spaces in file names with tabs.
             ;; The Java side must restore the spaces.
             (subst-char-in-string ?  ?\t file) 
             line))))
...
window to the top of the user's desktop."
  ...
  (if (not (buffer-modified-p))
      (progn
	(raise-frame)
	(matlab-eei-debug-mode-status t)
...
(defun matlab-eei-event-continue () 
  ...
  ;; (setq matlab-eei-matlab-stopped-p nil)
  (matlab-eei-debug-mode-status nil))
...
(defun matlab-eei-get-variable-value (variable)
  (matlab-eei-send-mcmd 
     (concat "display_variable " variable) t)    
  (if (string= (matlab-eei-wait-for-event) "variable-value")
      (unless (string= matlab-eei-last-variable "")
	    matlab-eei-last-variable)))

(defun matlab-eei-display-variable-tooltip (event)
  "Show tooltip for identifier where mouse EVENT occurred
if the debugger is active in."
  (when (and
	 matlab-eei-display-variable-tips-p
	 (eventp event)
	 (eq 
	  (tooltip-event-buffer event)
	  (marker-buffer overlay-arrow-position)))
    (let ((identifier (thing-at-point 'symbol)))
      (if (and
	   identifier
	   (not (string= identifier "")))
	  (let ((value (matlab-eei-get-variable-value identifier)))
	    (if value (tooltip-show value))))
      t)))
...
;; $Log: matlab-eei.el,v $
;; Revision 1.2.4.5  2006/06/27 21:22:39  batserve
;; 2006/06/18  1.2.4.3.2.2  batserve
;;   2006/06/15  1.2.4.3.2.1.4.1  jweng
;;     Updated copyright
;;   Accepted job 20909 in Ami
;; Accepted job 32269 in A
;;
;; Revision 1.2.4.3.2.2  2006/06/18 16:27:54  batserve
;; 2006/06/15  1.2.4.3.2.1.4.1  jweng
;;   Updated copyright
;; Accepted job 20909 in Ami
;;
;; Revision 1.2.4.3.2.1.4.1  2006/06/15 19:08:21  jweng
;; Updated copyright
;;
;; Revision 1.2.4.3.2.1  2005/12/19 14:31:44  batserve
;; 2005/11/30  1.2.4.3.22.1  paulk
;;   Related Records: 228279
;;   Code Reviewer: eludlam
;;   Enable variable display only if this version of Emacs supports tooltips.
;; 2005/12/02  1.2.4.3.22.2  paulk
;;   Related Records: 235230
;;   Code Reviewer: audreyb
;;   Request set and clear breakpoint functions now replace spaces in file
;;   names with tab characters to allow parsing of commands by Java side.
;; Accepted job 18411a in Ami
;;
;; Revision 1.2.4.3.22.2  2005/12/02 13:36:23  paulk
;; Related Records: 235230
;; Code Reviewer: audreyb
;; Request set and clear breakpoint functions now replace spaces in file
;; names with tab characters to allow parsing of commands by Java side.
;;
;; Revision 1.2.4.3.22.1  2005/11/30 16:00:55  paulk
;; Related Records: 228279
;; Code Reviewer: eludlam
;; Enable variable display only if this version of Emacs supports tooltips.
;;
;; Revision 1.2.2.2.2.2  2004/06/03 17:35:34  paulk
;; Related Records: 156741
;; Code Reviewer: audreyb
;; Use tooltips to display variable values as returned by datatipinfo.m.
;;
;; Revision 1.2.2.2.2.1  2004/05/26 16:01:05  paulk
;; Display variable values as tooltips in debug mode.
;;
;; Revision 1.2.2.2  2004/05/07 04:54:15  batserve
;; 2004/04/25  1.2.4.2  batserve
;;   2004/04/12  1.2.4.1.2.1  gweekly
;;     Updated copyright
;;   Accepted job 19572 in A
;; Accepted job 12391 in Amisc
;;
